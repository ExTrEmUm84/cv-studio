...
function ExperienceEditor({ item, onChange, onRemove }: { item: Experience; onChange: (item: Experience) => void; onRemove: () => void }) {
	return (
		<div className="cv-mini-card">
			...
			<Field label="Description" value={item.description} onChange={(description) => onChange({ ...item, description })} area />
			<button className="cv-link-button" onClick={onRemove}>Supprimer</button>
		</div>
	);
}
	...
	return (
		<aside className="cv-editor">
			...
			<section className="cv-panel">
				<h2>Design</h2>
				<label className="cv-field"><span>Modèle</span><select value={cv.template} onChange={(event) => set("template", event.target.value as CV["template"])}><option value="classic">Classique premium</option><option value="sidebar">Colonne latérale moderne</option><option value="minimal">Minimaliste</option></select></label>
			...
			<section className="cv-panel">
				<h2>Photo</h2>
				{cv.photo && <img className="cv-editor-photo" src={cv.photo} alt="Photo candidat" />}
				<label className="cv-file-button">Ajouter / changer la photo<input type="file" accept="image/png,image/jpeg,image/webp" onChange={(event) => onPhoto(event.target.files?.[0])} /></label>
				{cv.photo && <button className="cv-link-button" onClick={() => set("photo", "")}>Retirer la photo</button>}
			</section>
...
function SidebarInfo({ cv }: { cv: CV }) {
	return (
		<aside className="cv-preview-side">
			{cv.photo && <img className="cv-photo" src={cv.photo} alt="Photo candidat" />}
			<h2>Coordonnées</h2>
			<p>{cv.email}<br />{cv.phone}<br />{cv.city}<br />{cv.nationality}</p>
			<h2>Langues</h2>
			<div className="cv-side-langs">{lines(cv.languages).map((line) => { const lang = parseLanguage(line); return <p key={line}><strong>{lang.name}</strong><span>{lang.level}</span></p>; })}</div>
			<h2>Compétences</h2>
			<div className="cv-side-tags">{lines(cv.skills).map((skill) => <span key={skill}>{skill}</span>)}</div>
			<h2>Intérêts</h2>
		...
		<div className={`cv-paper cv-${cv.template}`} style={{ "--accent": cv.accent } as React.CSSProperties}>
			...
			<main className="cv-preview-main">
				<header className="cv-head">
					{cv.template !== "sidebar" && cv.photo && <img className="cv-photo cv-photo-inline" src={cv.photo} alt="Photo candidat" />}
					<div><h1>{cv.name}</h1><p className="cv-role">{cv.title}</p>{cv.template !== "sidebar" && <p className="cv-contact">{cv.email} · {cv.phone} · {cv.city} · {cv.nationality}</p>}</div>
				</header>
				...
				{cv.template !== "sidebar" && <section><h2>Compétences</h2><div className="cv-tags">{lines(cv.skills).map((skill) => <span key={skill}>{skill}</span>)}</div></section>}
				<section><h2>Formation</h2>{cv.education.map((item, index) => <div className="cv-item" key={index}><h3>{item.degree}</h3><small>{item.school} · {item.date} · {item.place}</small></div>)}</section>
				{cv.template !== "sidebar" && <section><h2>Langues</h2><div className="cv-languages">{lines(cv.languages).map((line) => { const lang = parseLanguage(line); return <div key={line}><strong>{lang.name}</strong><span>{lang.level}</span></div>; })}</div></section>}
				{cv.template !== "sidebar" && <section><h2>Intérêts</h2><p>{lines(cv.interests).join(" · ")}</p></section>}
			</main>
		</div>
	);
}
...
function CVPdf({ cv }: { cv: CV }) {
	return (
		<Document title={cv.name || "CV"}><Page size="A4" style={pdfStyles.page}>
			<View style={pdfStyles.row}>{cv.photo && <Image src={cv.photo} style={pdfStyles.photo} />}<View><Text style={[pdfStyles.h1, { color: cv.accent }]}>{cv.name}</Text><Text style={pdfStyles.role}>{cv.title}</Text><Text style={pdfStyles.contact}>{cv.email} · {cv.phone} · {cv.city} · {cv.nationality}</Text></View></View>
			<Text style={[pdfStyles.sectionTitle, { color: cv.accent }]}>Profil</Text><Text>{cv.profile}</Text>
			<Text style={[pdfStyles.sectionTitle, { color: cv.accent }]}>Expériences</Text>{cv.experiences.map((item, index) => <View key={index} style={pdfStyles.item} wrap={false}><Text style={pdfStyles.itemTitle}>{item.role} — {item.company}</Text><Text style={pdfStyles.muted}>{item.date} · {item.place}</Text><Text>{item.description}</Text></View>)}
			<Text style={[pdfStyles.sectionTitle, { color: cv.accent }]}>Compétences</Text><View style={pdfStyles.tagRow}>{lines(cv.skills).map((skill) => <Text key={skill} style={pdfStyles.tag}>{skill}</Text>)}</View>
			<Text style={[pdfStyles.sectionTitle, { color: cv.accent }]}>Formation</Text>{cv.education.map((item, index) => <View key={index} style={pdfStyles.item} wrap={false}><Text style={pdfStyles.itemTitle}>{item.degree}</Text><Text style={pdfStyles.muted}>{item.school} · {item.date} · {item.place}</Text></View>)}
			<Text style={[pdfStyles.sectionTitle, { color: cv.accent }]}>Langues</Text>{lines(cv.languages).map((line) => { const lang = parseLanguage(line); return <View key={line} style={pdfStyles.lang} wrap={false}><Text style={pdfStyles.itemTitle}>{lang.name}</Text><Text style={pdfStyles.muted}>{lang.level}</Text></View>; })}
		</Page></Document>
	);
}
